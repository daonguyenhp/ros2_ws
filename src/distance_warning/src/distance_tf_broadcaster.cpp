...
#include <tf2_ros/transform_broadcaster.h>          // Import the TransformBroadcaster class from TF2, used to publish data to the TF coordinate tree
...
class DistanceTfBroadcaster : public rclcpp::Node
{
    public:
        DistanceTfBroadcaster() : Node("distance_tf_broadcaster")
        {
            ...
            subscription_ = this->create_subscription<std_msgs::msg::Float32>(
                "/distance_topic",
                10,                 // Queue size (QoS default is Reliable)
    ...
    private:
        void broadcastCallback(const std_msgs::msg::Float32::SharedPtr msg)
        {
            ...
            // Position base_link exactly at the origin of the world coordinate system (0,0,0). The robot remains stationary.
