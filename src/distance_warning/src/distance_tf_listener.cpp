...
class DistanceTfListener : public rclcpp::Node
{
    public:
        DistanceTfListener() : Node("distance_tf_listener")
        {
            ...
            // TODO: Create a 1 Hz timer to call lookupAndCheck
    ...
    private:
        void lookupAndCheck()
        {
            try {
                ...
                // TODO: In khoảng cách
                RCLCPP_INFO(this->get_logger(), "TF distance world->sensor_link: %.2f m", dist);
                // TODO: Nếu dist > tf_threshold → gọi callSetThreshold(false)
                double current_threshold = this->get_parameter("tf_threshold").as_double();

                if (dist > current_threshold) {
                    RCLCPP_WARN(this->get_logger(), "Sensor out of range! %.2f m > tf_threshold %.2f m", dist, current_threshold);
                    this->callSetThreshold(false);
                }
